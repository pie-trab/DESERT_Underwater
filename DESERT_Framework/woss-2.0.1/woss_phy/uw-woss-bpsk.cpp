...
int UwMPhyBpskTransducer::command(int argc, const char*const* argv) {
  if( argc == 5 ) {
    if(strcasecmp(argv[1], "setTransducerType") == 0) {
      ...
      if (debug_)
        std::cout << NOW << "  UwMPhyBpskTransducer::command() setTransducerType called, low freq " << argv[2]
                  << "; high freq " << argv[3] << "; type " << argv[4] << std::endl;

      freq_transd_map[std::make_pair( atof(argv[2]), atof(argv[3]) )] = 
              std::move( woss::SWossController::instance().getTransducerHandler()->getValue( argv[4] ) );
...
std::shared_ptr<woss::Transducer> UwMPhyBpskTransducer::getTransducer( double frequency ) const {
  if ( freq_transd_map.empty() ) 
    return nullptr;

  for ( const auto&  it : freq_transd_map ) {
    if ( frequency >= it.first.first && frequency <= it.first.second ) {

      if (debug_) 
        cout << NOW << "  UwMPhyBpskTransducer::getTransducer() frequency " << frequency << " is in " 
...
//   double bwidth = spectralmask_->getBandwidth();
  ...
  if ( CentralFreqOptimization_ ) 
    frequency = txsmask.getFreq();

//   if ( BandwidthOptimization_ ) 
//     bwidth = txsmask.getBandwith();

  auto transducer = getTransducer( frequency );
  ...
  if ( transducer != nullptr && transducer->isValid() ) {
    ...
    if (debug_) 
      cout << NOW << "  UwMPhyBpskTransducer::consumedEnergyTx() transducer = " << transducer->getTypeName()
...
//       bwidth = txsmask.getBandwith();
    ...
    if ( transducer != nullptr && transducer->isValid() ) {
      ...
      if (debug_)
        cout << NOW << "  WossMPhyBpsk(" << mac_addr << ")::getTxPower() MaxTxSPL_dB_ = " << MaxTxSPL_dB_
...
//     hdr_cmn* ch = HDR_CMN(p);
    ...
    sendSyncClMsg(&m);
    auto channel = m.getTimeArr();

    if (channel == nullptr) {  
      if (debug_) 
        cout << NOW << "  WossMPhyBpsk(" << mac_addr << ")::getTxPower() no estimation info, tx at max power = " 
             << max_SPL << endl;
      ...
      auto press_temp = woss::SDefHandler::instance().createPressure( *channel );

      double attenuation = std::pow( press_temp->abs(), 2.0 );

      if ( attenuation <= 0.0 ) {
        if (debug_) 
          cout << NOW << "  WossMPhyBpsk(" << mac_addr << ")::getTxPower() no reception possible, tx at max power = " 
               << max_SPL << endl;
     ...
     if (debug_) 
      cout << NOW << "  WossMPhyBpsk(" << mac_addr << ")::getTxPower() SPL from formula and after SPL margin = " 
           << 10.0*log10(Ptx) << endl;
  //     //sanity check
  ...
  //     << std::endl;
      ...
      if (debug_) 
        cout << NOW << "  WossMPhyBpsk(" << mac_addr << ")::getTxPower() final spl = " 
