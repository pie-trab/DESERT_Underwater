...
int BathyGebcoDbCreatorTcl::command(int argc, const char*const* argv) {
  if ( argc == 3 ) {
    if(strcasecmp(argv[1], "setDbPathName") == 0) {
      ...
      if (debug) 
        std::cout << "BathyGebcoDbCreatorTcl::command() setDbPathName " << argv[2] << " called"  << std::endl;
    ...
    if(strcasecmp(argv[1], "useOneMinutePrecision") == 0) {
      if (debug) 
        std::cout << "BathyGebcoDbCreatorTcl::command() useOneMinutePrecision called"  << std::endl;

      gebco_type = GEBCO_BATHY_TYPE::GEBCO_1D_1_MINUTE_BATHY_TYPE;

      return TCL_OK;
    }
    else if(strcasecmp(argv[1], "useThirtySecondsPrecision") == 0) {
      if (debug) 
        std::cout << "BathyGebcoDbCreatorTcl::command() useThirtySecondsPrecision called"  << std::endl;

      gebco_type = GEBCO_BATHY_TYPE::GEBCO_1D_30_SECONDS_BATHY_TYPE;

      return TCL_OK;
    }
    else if(strcasecmp(argv[1], "use2DOneMinutePrecision") == 0) {
      if (debug) 
        std::cout << "BathyGebcoDbCreatorTcl::command() use2DOneMinutePrecision called"  << std::endl;

      gebco_type = GEBCO_BATHY_TYPE::GEBCO_2D_1_MINUTE_BATHY_TYPE;

      return TCL_OK;
    }
    else if(strcasecmp(argv[1], "use2DThirtySecondsPrecision") == 0) {
      if (debug) 
        std::cout << "BathyGebcoDbCreatorTcl::command() use2DThirtySecondsPrecision called"  << std::endl;
      ...
      if (debug) 
        std::cout << "BathyGebcoDbCreatorTcl::command() use2DFifteenSecondsPrecision called"  << std::endl;
