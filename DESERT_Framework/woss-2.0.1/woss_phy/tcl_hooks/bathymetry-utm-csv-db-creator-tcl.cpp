...
int BathyUtmCsvDbCreatorTcl::command(int argc, const char*const* argv) {
  if ( argc == 3 ) {
    if(strcasecmp(argv[1], "setDbPathName") == 0) {
      ...
      if (debug) 
        std::cout << "BathyUtmCsvDbCreatorTcl::command() setDbPathName " << argv[2] << " called"  << std::endl;
      ...
      if (debug) 
        std::cout << "BathyUtmCsvDbCreatorTcl::command() setCSVSeparator " << argv[2] << " called"  << std::endl;
