...
public:
  ...
  TclObject* create(int, const char*const*) {
    return( Singleton<WossDbManagerTcl>::instance_ptr() );
  }
...
int WossDbManagerTcl::command( int argc, const char*const* argv ) {
  ...
  if ( argc == 13 ) {
    if(strcasecmp(argv[1], "setCustomSediment") == 0) { // setCustomSediment tx_lat tx_long bearing range depth sedim_name v_p v_s dens a_p a_s
      ...
      auto ptr = SDefHandler::instance().createSediment( argv[6], atof(argv[7]), atof(argv[8]), atof(argv[9]), 
                                                         atof(argv[10]), atof(argv[11]), atof(argv[12]) );

      bool done = setCustomSediment( *ptr, Coord(atof(argv[2]), atof(argv[3])), bearing, range );
        
      if (debug) 
        std::cout << "WossDbManagerTcl::command() setCustomSediment called, tx latitude = " 
    ...
    if(strcasecmp(argv[1], "setCustomSediment") == 0) { // setCustomSediment sedim_name v_p v_s dens a_p a_s   
      auto ptr = SDefHandler::instance().createSediment( argv[2], atof(argv[3]), atof(argv[4]), atof(argv[5]), 
                                                         atof(argv[6]), atof(argv[7]), atof(argv[8]) );

      if (debug) 
        std::cout << "WossDbManagerTcl::command() setCustomSediment called "
    ...
    if(strcasecmp(argv[1], "setCustomAltimetry") == 0) { // setCustomAltimetry lat long bearing range ssp_filename_path
      ...
      if ( bearing > 180.0 || bearing < -180.0 ) 
        bearing = CCAltimetry::DB_CDATA_ALL_MEDIUM_KEYS;
      else 
        bearing = bearing * M_PI / 180.0;

      if (debug) 
        std::cout << "WossDbManagerTcl::command() setCustomAltimetry called, tx_lat = " 
                  << atof(argv[2]) << "; tx_long = " << atof(argv[3]) << "; bearing = " << bearing 
                  << "; range " << atof(argv[5]) << std::endl;

      auto altim_val = dynamic_cast< Altimetry* >( tcl.lookup(argv[2]) );
      
      if (altim_val) {
        setCustomAltimetry( *altim_val, Coord(atof(argv[2]), atof(argv[3])), atof(argv[4]), atof(argv[5]) );
        return TCL_OK;
      }
      else 
        return TCL_ERROR;
    ...
    if(strcasecmp(argv[1], "setCustomSSP") == 0) { // setCustomSSP lat long bearing Time ssp_filename_path
      ...
      if ( ( time == nullptr ) || (!time->isValid()) ) 
        time = &CCSSP::DB_CDATA_ALL_TIME_KEYS;
        
      if (debug) 
        std::cout << "WossDbManagerTcl::command() setCustomSSP called, tx_lat = " 
                  << atof(argv[2]) << "; tx_long = " << atof(argv[3]) << "; bearing = " << bearing 
    ...
    if(strcasecmp(argv[1], "setCustomSSP") == 0) { // setCustomSSP filename
      ...
      if (debug) 
        std::cout << "WossDbManagerTcl::command() setCustomSSP called, time = " << *time
      ...
      if (debug) 
        std::cout << "WossDbManager::command() getBathymetry called, latitude = " << atof(argv[2]) 
    ...
    if(strcasecmp(argv[1], "setCustomAltimetry") == 0) { // setCustomAltimetry filename
      ...
      if ( altim_val != nullptr ) {
        if (debug) 
          std::cout << "WossDbManagerTcl::command() setCustomAltimetry called = " << *altim_val << std::endl;

        bool valid = setCustomAltimetry(*altim_val);
      ...
      if (debug) 
        std::cout << "WossDbManagerTcl::command() setCustomAltimetry null pointer"  << std::endl;
    ...
    if(strcasecmp(argv[1], "closeAllConnections") == 0) {
      if (debug) 
        std::cout << "WossDbManager::command() closeAllConnections called"  << std::endl;
  ...
  if(strcasecmp(argv[1], "setCustomBathymetry") == 0) { // setCustomBathymetry lat long bearing total_ranges range bathy_value ...
    ...
    double bearing = atof( argv[4] );
    if ( bearing > 180.0 || bearing < -180.0 ) 
      bearing = CCBathymetry::DB_CDATA_ALL_MEDIUM_KEYS; 
    else 
      bearing = bearing * M_PI / 180.0;
    ...
    if (debug) 
      std::cout << "WossDbManagerTcl::command() setCustomBathymetry called, tx latitude = " << atof(argv[2]) 
    ...
    if ( total_values == 0 ) {
      ...
      for ( int i = 0; i < 2*total_values; i += 2 ) {
        ...
        if (debug) 
          std::cout << "WossDbManagerTcl::command() setCustomBathymetry range = " << atof(argv[range]) << "; bathymetry = "
