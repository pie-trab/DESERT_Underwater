...
static constexpr inline double ARR_ASC_RES_NOT_SET = (-4000.0);
...
int ArrData::getTimeArrIndex( double tx_depth, double rx_depth, double rx_range ) const {
  ...
  int rx_depth_index = getIndex( rx_depth, rx_depths.get(), Nrd );
  int rx_range_index = getIndex( rx_range, rx_ranges.get(), Nrr );
...
ArrAscResReader::ArrAscResReader( const Woss& woss )
: ResReader(woss),
...
bool ArrAscResReader::getArrAscHeader() {
  if ( arr_asc_header_collected == true ) 
  ...
  if(!file_reader) {
    if (woss_ref.usingDebug()) 
      std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getArrAscHeader() WARNING, can't open " << file_name << std::endl;
    return false;
  }
  ...
  if (woss_ref.usingDebug())
    std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getArrAscHeader() syntax = "
  ...
  if (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_2) {
    ...
    if(sim_type.compare("'2D'") == 0) {
      ...
      for (int i = 0; i < arr_file.Nsd; i++) {
          ...
          if (woss_ref.usingDebug()) 
            std::cout << "ArrAscResReader(" << woss_ref.getWossId()
      ...
      for (int i = 0; i < arr_file.Nrd; i++) {
        ...
        if (woss_ref.usingDebug()) 
          std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getArrAscHeader() Reading rx depth(" << i << "/"
      ...
      for (int i = 0; i < arr_file.Nrr; i++) {
        ...
        if (woss_ref.usingDebug()) 
          std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getArrAscHeader() Reading rx range(" << i << "/"
    ...
    if (woss_ref.usingDebug()) 
      std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getArrAscHeader() freq = " << arr_file.frequency << "; Nsd = " << arr_file.Nsd
    ...
    for (int i = 0; i < arr_file.Nsd; i++) {
      ...
      if (woss_ref.usingDebug()) 
      std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() Reading source depth(" << i << "/"
                << i << ") = " << arr_file.tx_depths[i] << std::endl;
    }
    ...
    for (int i = 0; i < arr_file.Nrd; i++) {
      ...
      if (woss_ref.usingDebug()) 
        std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() Reading rx depth(" << i << "/"
                  << i << ") = " << arr_file.rx_depths[i] << std::endl;
    }
    ...
    for (int i = 0; i < arr_file.Nrr; i++) {
      ...
      if (woss_ref.usingDebug()) 
      std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() Reading rx range(" << i << "/"
                << i << ") = " << arr_file.rx_ranges[i] << " [m]" << std::endl;
    }
  }
...
bool ArrAscResReader::getArrAscFile() {
  ...
  for (int isd = 0; isd < arr_file.Nsd ; isd++) {
    ...
    if (woss_ref.usingDebug())
      std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() Reading data for source " << isd << " of "
    ...
    for (int ird = 0; ird < arr_file.Nrd; ird++) {
      for (int irr = 0; irr < arr_file.Nrr; irr++ ) {
        ...
        if ( woss_ref.usingDebug() )
          std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() curr_arrivals = "
        ...
        if (curr_arrivals > 0) {
          ...
          for (int i = 0; i < curr_arrivals; i++) {
            ...
            if (woss_ref.usingDebug())
              std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() s_depth = " << arr_file.tx_depths[isd]
                        ...
                        << "; exp = " << (bwoss_ref.getBellhopArrSyntax() != woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_0 ? std::exp(-2.0 * M_PI * arr_file.frequency * curr_delay_imag) : 1.0)
          ...
          if ( woss_ref.usingDebug() )
            std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getAsciiArrMap() no arrivals, inserted zero value"
                      << std::endl;
        }
      }
    }
...
std::unique_ptr<Pressure> ArrAscResReader::readPressure(double frequency, double tx_depth, double rx_depth, double rx_range ) const {
  return( SDefHandler::instance().createPressure( *readTimeArr( frequency, tx_depth, rx_depth, rx_range ) ) );
}


std::unique_ptr<Pressure> ArrAscResReader::readAvgPressure( double frequency, double tx_depth, double start_rx_depth, double start_rx_range, double end_rx_depth, double end_rx_range ) {
  if ( !arr_asc_file_collected ) return SDefHandler::instance().createPressure( Pressure::createNotValid() );
...
std::unique_ptr<TimeArr> ArrAscResReader::readTimeArr( double frequency, double source_depth, double rx_depth, double rx_range ) const {
  if ( !arr_asc_file_collected ) return( SDefHandler::instance().createTimeArr( TimeArr::createNotValid() ) );
...
std::complex<double> ArrAscResReader::readMapAvgPressure( double frequency, double tx_depth, double start_rx_depth, double start_rx_range, double end_rx_depth, double end_rx_range ) {
  if ( ( last_tx_depth == tx_depth ) && ( last_start_rx_depth == start_rx_depth ) && ( last_start_rx_range == start_rx_range )
   &&  ( last_end_rx_depth == end_rx_depth ) && ( last_end_rx_range == end_rx_range ) )
     ...
     if (woss_ref.usingDebug()) 
      std::cout << "ArrAscResReader(" << woss_ref.getWossId() << ")::getMapAvgPressure() sd = " << tx_depth
