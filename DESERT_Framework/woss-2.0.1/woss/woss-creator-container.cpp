...
std::shared_ptr<Location> WossCreatorContainer< CustomTransducer >::ALL_LOCATIONS = nullptr;
...
WossCreatorContainer< CustomTransducer >::DCIter WossCreatorContainer< CustomTransducer >::find( const CoordZ& coordinates ) {
  ...
  for ( auto it = data_container.begin(); it != data_container.end(); it++ ) {
    ...
    if ( it->first->isEquivalentTo( coordinates ) ) {
      if (debug) 
        std::cout << "WossCreatorContainer<CustomTransducer>::find() tx coordinates found = " 
  ...
  if ( coordinates == ALL_COORDZ && it_all_loc != data_container.end() ) {
    if (debug) 
      std::cout << "WossCreatorContainer<CustomTransducer>::find() tx ALL_COORDZ found" << std::endl;
    return it_all_loc;
  }
  
  if (debug && coordinates != ALL_COORDZ ) 
    std::cout << "WossCreatorContainer<CustomTransducer>::find() tx coordinates not found = " 
              << coordinates << std::endl; 
  
  if (debug && coordinates == ALL_COORDZ ) 
    std::cout << "WossCreatorContainer<CustomTransducer>::find() tx ALL_COORDZ not found" << std::endl;
...
WossCreatorContainer< CustomTransducer >::ICIter WossCreatorContainer< CustomTransducer >::find( const CoordZ& coordinates, const DCIter& iter ) {
  ...
  for ( auto it = iter->second.begin(); it != iter->second.end(); it++ ) {
    ...
    if ( it->first->isEquivalentTo( coordinates ) ) {
      if (debug) 
        std::cout << "WossCreatorContainer<CustomTransducer>::find() rx coordinates found = " 
                  << coordinates << std::endl;
      return it;
    }
  }
  ...
  if ( coordinates == ALL_COORDZ && it_all_loc != iter->second.end() ) {
    if (debug) 
      std::cout << "WossCreatorContainer<CustomTransducer>::find() rx ALL_COORDZ found" << std::endl;
    return it_all_loc;
  }
  
  if (debug && coordinates != ALL_COORDZ ) 
    std::cout << "WossCreatorContainer<CustomTransducer>::find() rx coordinates not found = " 
              << coordinates << std::endl; 
  
  if (debug && coordinates == ALL_COORDZ ) 
    std::cout << "WossCreatorContainer<CustomTransducer>::find() rx ALL_COORDZ not found" << std::endl;
  return iter->second.end(); 
}


CustomTransducer WossCreatorContainer< CustomTransducer >::get( const CoordZ& tx, const CoordZ& rx ) const { 
  auto it = const_cast< WossCreatorContainer< CustomTransducer >& >(*this).find( tx );
  ...
  if ( it != data_container.end() ) {
    auto it2 = const_cast< WossCreatorContainer< CustomTransducer >& >(*this).find( rx, it );
    
    if ( rx != ALL_COORDZ ) {
      if ( it2 == it->second.end() ) 
        it2 = const_cast< WossCreatorContainer< CustomTransducer >& >(*this).find( ALL_COORDZ, it );
    }
        
    if ( it2 != it->second.end() ) {
      if ( tx_changed_to_all_coordz ) 
        return CustomTransducer(it2->second);
      else {
        if ( debug ) 
          std::cout << "WossCreatorContainer<CustomTransducer>::get() rotating angle" << std::endl;
...
CustomTransducer WossCreatorContainer< CustomTransducer >::get( const std::shared_ptr<Location>& tx, const std::shared_ptr<Location>& rx ) const { 
  auto it = const_cast< WossCreatorContainer< CustomTransducer >::DataContainer& >(data_container).find( tx );
  bool tx_changed_to_all_locations = false;

  if ( tx != ALL_LOCATIONS ) {
    if ( it == data_container.end() ) {
      if ( debug ) 
        std::cout << "WossCreatorContainer<CustomTransducer>::get() no tx location found = " << *tx 
    ...
    if ( debug ) 
      std::cout << "WossCreatorContainer<CustomTransducer>::get() tx location found = " << *tx << std::endl;
  }
  ...
  if ( it != data_container.end() ) {
    ...
    if ( rx != ALL_LOCATIONS ) {
      ...
      if ( it2 == it->second.end() ) {
        if ( debug ) 
          std::cout << "WossCreatorContainer<CustomTransducer>::get() no rx location found = " << *rx 
                    << "; trying ALL_LOCATIONS"<< std::endl;
      ...
      if ( debug ) 
        std::cout << "WossCreatorContainer<CustomTransducer>::get() rx location found = " << *rx << std::endl;
    }

    if ( it2 != it->second.end() ) {
      if ( tx_changed_to_all_locations ) 
        return CustomTransducer( it2->second );
      else {
        if ( debug ) 
          std::cout << "WossCreatorContainer<CustomTransducer>::get() rotating angle" << std::endl;
  ...
  if ( debug ) 
    std::cerr << "WARNING: WossCreatorContainer<CustomTransducer>::get() no tx nor rx location found, returning default constructor!!" << std::endl;
