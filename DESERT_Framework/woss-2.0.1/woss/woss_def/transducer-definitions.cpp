...
static constexpr inline int TRANSDUCER_WRITE_PRECISION = ( 17 ); 
static constexpr inline int TRANSDUCER_WRITE_MARGIN = ( TRANSDUCER_WRITE_PRECISION + 9 );
  ...
  type_name()
{
  ...
  if ( !beam_power_map.empty() ) 
    beam_precision = beam_power_map.begin()->first.getPrecision();
  if ( !conductance_map.empty() ) 
    conductance_precision = conductance_map.begin()->first.getPrecision();
  if ( !tvr_map.empty() ) 
    tvr_precision = tvr_map.begin()->first.getPrecision();
  if ( !ocv_map.empty() ) 
    ocv_precision = ocv_map.begin()->first.getPrecision();
}


std::unique_ptr<Transducer> Transducer::create( long double beam_prec, long double cond_prec, long double tvr_prec, long double ocv_prec ) const {
  return std::make_unique<Transducer>( beam_prec, cond_prec, tvr_prec, ocv_prec );
}


std::unique_ptr<Transducer> Transducer::create( BeamPowerMap& beam_map, ConductanceMap& cond_map, TVRMap& t_map, OCVMap& o_map ) const {
  return std::make_unique<Transducer>( beam_map, cond_map, t_map, o_map );
}


std::unique_ptr<Transducer> Transducer::clone() const {
  return std::make_unique<Transducer>( *this );
}
...
Transducer& Transducer::setBeamPrecision( long double prec ) {
  if ( prec == beam_precision ) 
    return *this;
  
  BeamPowerMap beam_power_temp;
  
  for ( const auto& it : beam_power_map ) {
    beam_power_temp.emplace( std::make_pair( PDouble( it.first.getValue(), prec), it.second ) );
  }
...
Transducer& Transducer::setOCVPrecision( long double prec ) {
  ...
  for ( const auto& it : ocv_map ) {
    ocv_temp.emplace( std::make_pair( PDouble( it.first.getValue(), prec), it.second ) );
  }
...
Transducer& Transducer::setConductancePrecision( long double prec ) {
  if ( prec == conductance_precision ) 
    return *this;
  
  ConductanceMap conductance_temp;
  
  for ( const auto& it : conductance_map ) {
    conductance_temp.emplace( std::make_pair( PDouble( it.first.getValue(), prec), it.second ) );
  }
...
Transducer& Transducer::setTVRPrecision( long double prec ) {
  if ( prec == tvr_precision ) 
    return *this;
  
  TVRMap tvr_temp;
  
  for ( const auto it : tvr_map ) {
    tvr_temp.emplace( std::make_pair( PDouble( it.first.getValue(), prec), it.second ) );
  }
...
const std::string& Transducer::getSymmetryString() const {
  if (has_conical_symmetry) 
    return conical_string;
  else 
    return toroidal_string;
}
...
void Transducer::beampattern_rotate( double angle, BeamPowerMap& map ) {
  ...
  if ( min_angle != -180.0 || max_angle != 180.0 ) {
    beam_power_temp[PDouble(-180.0, beam_precision) ] = 10.0*log10( std::abs(min_angle_value_lin - (180.0 - std::abs(min_angle)) * (delta_value_lin / delta_angle)) );
    beam_power_temp[PDouble(180.0, beam_precision) ] = 10.0*log10( std::abs(max_angle_value_lin + (180.0 - std::abs(max_angle)) * ( delta_value_lin / delta_angle)) );
  }
...
double Transducer::getValue( double key, const std::map< PDouble, double >& map, long double precision, bool use_linear, double lin_costant ) const {
  ...
  if (debug) 
    std::cout << "Transducer::getValue() key " << key << "; prec " << precision 
  ...
  if ( tvr_map.size() == 1 ) {
    ...
    if ( use_linear ) 
      ret_val = std::pow( 10.0, ret_val/lin_costant );
    
    if (debug) 
      std::cout << "Transducer::getValue() map has one value " << ret_val << std::endl;    
  }
  else {
    ...
    if ( it == map.end() ) {
      ...
      if (debug) 
        std::cout << "Transducer::getValue() all map's values have key < " << key << "; ret val " << ret_val << std::endl;          
    }
      ...
      if (debug)
        std::cout << "Transducer::getValue() key value found or map begin()  " << key << "; ret val " << ret_val << std::endl;                
    }
    else {
      before_it = it;
      before_it--;
      ...
      if (debug) 
        std::cout << "Transducer::getValue() frequency value made from: prev key " << (double)before_it->first 
...
double Transducer::getSPL( double frequency, double power ) const {
  ...
  if (debug)
     std::cout << "Transducer::getSPL() freq " << frequency << "; power " << power
...
double Transducer::getPowerFromSPL( double frequency, double spl ) const {  
  ...
  if (debug) 
    std::cout << "Transducer::getPowerFromSPL() freq " << frequency << "; spl " << spl
              << "; tvr " << tvr << "; g " << g 
...
bool Transducer::import( std::istream& stream_in ) {
  ...
  if (debug) 
    std::cout << "Transducer::import() type name " << type_name << "; symmetry " << getSymmetryString()
...
bool Transducer::importBinary( std::fstream& file_in ) {
  ...
  if ( debug )
    std::cout << "Transducer::importBinary() type name " << type_name << "; symmetry " << getSymmetryString()
              << "; res freq " << resonance_frequency
              << "; bandwith " << bandwith_3db << "; max pow " << max_power << "; duty cycle " << duty_cycle
              << "; tvr prec " << tvr_precision << "; ocv prec " << ocv_precision 
              << "; cond prec " << conductance_precision << "; beam prec " << beam_precision << std::endl;  
...
bool Transducer::import( std::istream& stream_in, std::map< PDouble, double >& map, long double prec, bool is_angle ) { 
  ...
  while ( value_counter != total_values ) {
    ...
    if ( debug ) 
      std::cout << "Transducer::import() key = " << curr_key << "; value = " << curr_value 
...
bool Transducer::importBinary( std::fstream& file_in, std::map< PDouble, double >& map, long double prec, bool is_angle ) { 
  ...
  while ( value_counter != total_values ) {
    ...
    if (debug) 
      std::cout << "Transducer::importBinary() key = " << curr_key << "; value = " << curr_value 
                << "; cnt = " << value_counter << "; total values = " << total_values << std::endl; 
  }
...
bool Transducer::writeBinary( std::fstream& file_out, const std::map< PDouble, double >& map ) const {
  ...
  for ( const auto& [key, value] : map ) {
    double key_val = key.getValue();
    file_out.write( reinterpret_cast< const char* > (&key_val), sizeof(double) );
    file_out.write( reinterpret_cast< const char* > (&value), sizeof(double) );
  }
...
bool Transducer::writeVertBeamPattern( std::ostream& stream_out, const CoordZ& tx, const CoordZ& rx, double init_bearing, double vert_rot, double horiz_rot, double mult_costant, double add_costant ) const {
  if (debug) {
    std::streamsize old_prec = std::cout.precision(TRANSDUCER_WRITE_PRECISION);
  ...
  auto [rx_rho_primo, rx_theta_primo, rx_phi_primo] = convCart2Spher( rx_x_primo, rx_y_primo, rx_z_primo );

  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() rx_x_primo " << rx_x_primo << "; rx_y_primo " << rx_y_primo 
  ...
  auto [rx_x_secondo, rx_y_secondo, rx_z_secondo] = convSpher2Cart( rx_rho_primo, rx_theta_primo, rx_phi_primo );

  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() rx_x_secondo " << rx_x_secondo << "; rx_y_secondo " << rx_y_secondo 
  ...
  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() alpha " << alpha*180.0/M_PI 
              << "; rot_angle " << rot_angle*180.0/M_PI << std::endl;
  
  auto [ rx_x_terzo, rx_y_terzo ] = rotCartXY( rot_angle, rx_x_secondo, rx_y_secondo );
  auto rx_z_terzo = rx_z_secondo;
  ...
  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() rx_x_terzo " << rx_x_terzo << "; rx_y_terzo " << rx_y_terzo 
  ...
  double  rx_y_quarto, tx_bw_y_quarto;
  auto [ rx_x_quarto, rx_z_quarto ] = rotCartXZ( rot_angle2, rx_x_terzo, rx_z_terzo  );
  auto [ tx_bw_x_quarto, tx_bw_z_quarto ] = rotCartXZ( rot_angle2, tx_bw_x_terzo, tx_bw_z_terzo );
  rx_y_quarto = rx_y_terzo;
  tx_bw_y_quarto = tx_bw_y_terzo;

  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() rot_angle2 " << rot_angle2*180.0/M_PI << "; rx_x_quarto " << rx_x_quarto 
  ...
  if (debug)
    std::cout << "Transducer::writeVertBeamPattern() rx_x_quinto " << rx_x_quinto 
  ...
  auto [nv_x, nv_y, nv_z] = crossProd( rx_x_quinto, rx_y_quinto, rx_z_quinto, tx_bw_x_quinto, tx_bw_y_quinto, tx_bw_z_quinto );

  auto norm_nv = normCart( nv_x, nv_y, nv_z );
  nv_x = nv_x / norm_nv;
  nv_y = nv_y / norm_nv;
  nv_z = nv_z / norm_nv;
  
  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() nv_x " 
...
//   else scalar_thresh = 1e-2;
  ...
  using TriMap = std::map< double, std::pair< double, double> >;
  using TMRIter = TriMap::reverse_iterator;
  using TMIter = TriMap::iterator;
  ...
  for ( double curr_theta = 0.0; curr_theta <= M_PI; curr_theta += theta_step ) {

    if ( has_conical_symmetry ) 
      curr_gain = getValue( curr_theta*180.0/M_PI, beam_power_map, beam_precision, true, 10.0 );
    else 
      curr_gain = getValue( (M_PI/2.0 - curr_theta)*180.0/M_PI, beam_power_map, beam_precision, true, 10.0 );

    for ( double curr_phi = -M_PI; curr_phi <= M_PI; curr_phi += phi_step ) {
      auto [curr_x, curr_y, curr_z] = convSpher2Cart( curr_gain, curr_theta, curr_phi );
 
      double curr_scal = scalarProd( nv_x, nv_y, nv_z, curr_x, curr_y, curr_z ) / normCart( curr_x, curr_y, curr_z );

      if (debug) 
        std::cout << "Transducer::writeVertBeamPattern() curr_theta " << curr_theta*180.0/M_PI 
      ...
      if ( std::abs(curr_scal) <= scalar_thresh ) {
        TMIter curr_iter = tri_map.emplace( std::make_pair( curr_phi, std::make_pair( curr_theta, curr_gain ) ) ).first;

        if (debug) 
          std::cout << "Transducer::writeVertBeamPattern() inserting curr_theta " << curr_theta*180.0/M_PI 
                    << "; curr_phi " << curr_phi*180.0/M_PI << "; curr_gain " << 10.0*log10( curr_gain ) 
                    << "; curr_scal " << curr_scal << std::endl;  
  ...
  if (debug) 
    std::cout << "Transducer::writeVertBeamPattern() trimap size " << tri_map.size() << std::endl;

  for ( auto it = tri_map.begin(); it != tri_map.end(); it++ ) {
    if (debug) 
      std::cout << "Transducer::writeVertBeamPattern() phi " << it->first*180.0/M_PI 
                << "; theta " << it->second.first*180.0/M_PI << "; gain db " << 10.0*log10( it->second.second ) << std::endl;
  }

  bool has_to_change_theta = false;
  for ( auto rit = min_iter; rit != tri_map.rend(); rit++ ) {
    if ( rit->second.first == curr_max_theta ) 
      has_to_change_theta = true;
    ...
    if (debug) 
      std::cout << "Transducer::writeVertBeamPattern() change theta " << has_to_change_theta 
  ...
  if ( min_iter != tri_map.rbegin() ) {
    for ( auto rit = tri_map.rbegin(); rit != min_iter; rit++ ) {
      if ( rit->second.first == curr_max_theta ) 
        has_to_change_theta = true;
  ...
  if (has_conical_symmetry) 
    const_cast< Transducer* > (this)->beampattern_rotate( vert_rot, beam_write_map );
  else 
    const_cast< Transducer* > (this)->beampattern_rotate( vert_rot - 90.0, beam_write_map );
