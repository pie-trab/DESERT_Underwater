...
static constexpr inline double ARR_BIN_RES_NOT_SET = (-4000.0);
...
bool ArrBinResReader::getArrBinHeader() {
  ...
  if (woss_ref.usingDebug())
    std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinHeader() syntax = "
              << (int) bwoss_ref.getBellhopArrSyntax() << std::endl;

  file_reader.seekg(4, std::ios_base::beg);

  if (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_2)
  {
    ...
    if(strncmp(sim_type, "'2D'", sizeof(sim_type)) == 0)
    {
      ...
      arr_file.tx_depths = std::move( std::make_unique< float[] >(arr_file.Nsd) );
      file_reader.read( reinterpret_cast<char*>(arr_file.tx_depths.get()), arr_file.Nsd * sizeof(float) );

      file_reader.ignore(8);
      //Read in num receivers depths and receiver depths
      file_reader.read(reinterpret_cast<char*>(&arr_file.Nrd),sizeof(int32_t));
      arr_file.rx_depths = std::move( std::make_unique< float[] >(arr_file.Nrd) );
      file_reader.read( reinterpret_cast<char*>(arr_file.rx_depths.get()), arr_file.Nrd * sizeof(float) );
      file_reader.ignore(8);

      //Read in num receivers ranges and receiver ranges
      file_reader.read(reinterpret_cast<char*>(&arr_file.Nrr),sizeof(int32_t));
      arr_file.rx_ranges = std::move( std::make_unique< float[] >(arr_file.Nrr) );

      auto rx_ranges_tmp = std::make_unique< double[] >(arr_file.Nrr);
      file_reader.read( reinterpret_cast<char*>(rx_ranges_tmp.get()), arr_file.Nrr * sizeof(double) );
  ...
  else
  {
    ...
    file_reader.seekg(8, std::ios_base::cur);
    file_reader.read( reinterpret_cast<char*>(arr_file.rx_ranges.get()), arr_file.Nrr * sizeof(float) );
  }

  if (woss_ref.usingDebug())
    std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinHeader() freq = " << arr_file.frequency << "; Nsd = " << arr_file.Nsd << "; Nrd = "
  ...
  if (woss_ref.usingDebug()) {
    for (int i = 0; i < arr_file.Nsd; i++) {
      std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinHeader() Nsd(" << i << ") = " << arr_file.tx_depths[i] << std::endl;
    }

    for (int i = 0; i < arr_file.Nrd; i++) {
      std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinHeader() Nrd(" << i << ") = " << arr_file.rx_depths[i] << std::endl;
    }

    for (int i = 0; i < arr_file.Nrr; i++) {
      std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinHeader() Nrr(" << i << ") = " << arr_file.rx_ranges[i] << std::endl;
    }
  }

  arr_file.arr_values = SDefHandler::instance().createTimeArrArray(arr_file.Nsd * arr_file.Nrd * arr_file.Nrr);
...
bool ArrBinResReader::getArrBinFile() {
  if ( !arr_bin_header_collected ) 
    return false;

  assert(skip_header > 0);

  if (woss_ref.usingDebug())
      std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinFile() skip_header = "
  ...
  for (int isd = 0; isd < arr_file.Nsd ; isd++) {
    ...
    if (woss_ref.usingDebug())
      std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinFile() Reading data for source "
    ...
    for (int ird = 0; ird < arr_file.Nrd; ird++) {
      for (int irr = 0; irr < arr_file.Nrr; irr++ ) {
        ...
        if ( woss_ref.usingDebug() )
          std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinFile() curr_arrivals = "
        ...
        if (curr_arrivals > 0) {
          ...
          for (int i = 0; i < curr_arrivals; i++) {
            ...
            if (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_1 ||
                bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_2) {
            ...
            std::unique_ptr<Pressure> curr_pressure_ptr = nullptr;

            if (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_0) {
              curr_pressure_ptr = SDefHandler::instance().createPressure( (curr_amplitude * cos(2.0 * M_PI * arr_file.frequency + curr_phase * M_PI / 180.0) ) ,
                                                                                 (curr_amplitude * sin(2.0 * M_PI * arr_file.frequency + curr_phase * M_PI / 180.0) ) );
            }
            else if (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_1 ||
                     bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_2) {
              curr_pressure_ptr = SDefHandler::instance().createPressure( (curr_amplitude * std::exp(2.0 * M_PI * arr_file.frequency * curr_delay_imag)
            ...
            if (woss_ref.usingDebug())
              std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinFile() s_depth = " << arr_file.tx_depths[isd]
                        << "; rx depth = " << arr_file.rx_depths[ird] << "; rx range = " << arr_file.rx_ranges[irr]
                        << " freq = " << arr_file.frequency << "; delay = " << curr_delay
                        << "; delay_imag = " << (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_1 ? curr_delay_imag : 0.0)
                        << "; amplitude = " << curr_amplitude << "; phase = " << curr_phase
                        << "; exp = " << (bwoss_ref.getBellhopArrSyntax() == woss::BellhopArrSyntax::BELLHOP_CREATOR_ARR_FILE_SYNTAX_1 ? std::exp(-2.0 * M_PI * arr_file.frequency * curr_delay_imag) : 1.0)
            ...
            curr_pressure_ptr.reset();
          }
        }
        else {
          arr_file.arr_values[isd * arr_file.Nrd * arr_file.Nrr + ird * arr_file.Nrr + irr].sumValue( 0.0 , *(SDefHandler::instance().createPressure( 0.0, 0.0 )) ) ;

          if ( woss_ref.usingDebug() )
            std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinFile() no arrivals, inserted zero value"
                      << std::endl;

        }
        if (woss_ref.usingDebug())
          std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getArrBinFile() chan = " 
                    << arr_file.arr_values[isd * arr_file.Nrd * arr_file.Nrr + ird * arr_file.Nrr + irr] << std::endl;
      }
    }
  }
  file_reader.close();
...
std::unique_ptr<Pressure> ArrBinResReader::readPressure( double frequency, double tx_depth, double rx_depth, double rx_range ) const {
  return( SDefHandler::instance().createPressure( *readTimeArr( frequency, tx_depth, rx_depth, rx_range ) ) );
}


std::unique_ptr<Pressure> ArrBinResReader::readAvgPressure( double frequency, double tx_depth, double start_rx_depth, double start_rx_range, double end_rx_depth, double end_rx_range ) {
  if ( !arr_bin_file_collected ) 
...
std::unique_ptr<TimeArr> ArrBinResReader::readTimeArr(double frequency, double source_depth, double rx_depth, double rx_range) const {
  if ( !arr_bin_file_collected ) 
    SDefHandler::instance().createTimeArr( TimeArr::createNotValid() );
  return( accessMap( frequency, source_depth,rx_depth,rx_range ).clone() );
}
...
std::complex<double> ArrBinResReader::readMapAvgPressure(double frequency, double tx_depth, double start_rx_depth, double start_rx_range, double end_rx_depth, double end_rx_range ) {
  if (     ( last_tx_depth == tx_depth ) && ( last_start_rx_depth == start_rx_depth ) && ( last_start_rx_range == start_rx_range )
    ...
    if (woss_ref.usingDebug()) 
    std::cout << "ArrBinResReader(" << woss_ref.getWossId() << ")::getMapAvgPressure() sd = " << tx_depth
