...
static constexpr inline double SHD_RES_NOT_SET = -4000.0;
...
int ShdData::getIndex( float value, float array[], int32_t array_size ) const {
  if ( value <= array[0] || array_size == 1 ) {
    ...
    if ( ( abs( quantized_value ) - floor( abs( quantized_value ) ) ) >= 0.5 ) 
      return( ceil( quantized_value ) );
    else 
      return( floor( quantized_value ) );
  }
}


int ShdData::getPressureIndex( double tx_depth, double rx_depth, double rx_range, double tx_theta ) const {
  int theta_index = getIndex( tx_theta, theta.get(), Ntheta );
  int tx_depth_index = getIndex( tx_depth, tx_depths.get(), Nsd );
  int rx_depth_index = getIndex( rx_depth, rx_depths.get(), Nrx_per_range );
  int rx_range_index = getIndex( rx_range/1000.0, rx_ranges.get(), Nrr );
...
int ShdData_v1::getPressureIndex( double tx_freq, double tx_depth, double rx_depth, double rx_range, double tx_theta ) const {
  int freq_index = getIndex( tx_freq, frequencies.get(), Nfreq );
  int theta_index = getIndex( tx_theta, theta.get(), Ntheta );
  int tx_depth_index = getIndex( tx_depth, tx_depths.get(), Nsd );
  int rx_depth_index = getIndex( rx_depth, rx_depths.get(), Nrx_per_range );
  int rx_range_index = getIndex( rx_range/1000.0, rx_ranges.get(), Nrr );
...
ShdResReader::ShdResReader( const Woss& woss ) 
:  ResReader(woss),
...
bool ShdResReader::getShdHeader() {
  ...
  const BellhopWoss& bwoss_ref = dynamic_cast<const BellhopWoss&>(woss_ref);

  file_reader.open(file_name.c_str() , std::ios::binary|std::ios::in);

  if(!file_reader) {
      if (woss_ref.usingDebug()) std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() WARNING, results file " << file_name 
				<< " non existant" << std::endl;
      return false;
  }

  if (bwoss_ref.getBellhopShdSyntax() == woss::BellhopShdSyntax::BELLHOP_CREATOR_SHD_FILE_SYNTAX_0) {
    file_reader.read(reinterpret_cast<char*>(&shd_file.record_length),sizeof(int32_t));

    if (woss_ref.usingDebug()) 
      std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() record length = " << shd_file.record_length << std::endl;

    file_reader.seekg(4*shd_file.record_length, std::ios_base::beg); //reposition to end of first record
    shd_file.plot_type = std::move( std::make_unique<char []>(10*sizeof(char*)) );
    file_reader.read(shd_file.plot_type.get(), 10*sizeof(char*));

    if (woss_ref.usingDebug()) 
      std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() plot type : " << shd_file.plot_type.get() << std::endl;
    ...
    if ( strcmp( shd_file.plot_type.get(), "rectilin  " ) == 0) {
        shd_file.Nrx_per_range = shd_file.Nrd;
    }
    else if ( strcmp( shd_file.plot_type.get(), "irregular " ) == 0) {
    ...
    if (woss_ref.usingDebug()) std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Ntheta = " << shd_file.Ntheta 
            << "; Nsd = " << shd_file.Nsd << "; Nrd = " << shd_file.Nrd << "; Nrr = " << shd_file.Nrr 
            << "; Nrx_range = " << shd_file.Nrx_per_range << std::endl;

    shd_file.theta = std::move( std::make_unique<float[]>(shd_file.Ntheta) );
    shd_file.tx_depths = std::move( std::make_unique<float[]>(shd_file.Nsd) );
    shd_file.rx_depths = std::move( std::make_unique<float[]>(shd_file.Nrd) );
    shd_file.rx_ranges = std::move( std::make_unique<float[]>(shd_file.Nrr) );
    shd_file.press_values =std::move( std::make_unique< std::complex<double>[] >(shd_file.Ntheta * shd_file.Nsd * shd_file.Nrd * shd_file.Nrr) );

    file_reader.seekg(3*4*shd_file.record_length, std::ios_base::beg); //reposition to end of record 3
    file_reader.read(reinterpret_cast<char*>(shd_file.theta.get()),shd_file.Ntheta*sizeof(float));

    file_reader.seekg(4*4*shd_file.record_length, std::ios_base::beg); //reposition to end of record 4
    file_reader.read(reinterpret_cast<char*>(shd_file.tx_depths.get()),shd_file.Nsd*sizeof(float));

    file_reader.seekg(5*4*shd_file.record_length, std::ios_base::beg); //reposition to end of record 5
    file_reader.read(reinterpret_cast<char*>(shd_file.rx_depths.get()),shd_file.Nrd*sizeof(float));

    file_reader.seekg(6*4*shd_file.record_length, std::ios_base::beg); //reposition to end of record 6
    file_reader.read(reinterpret_cast<char*>(shd_file.rx_ranges.get()),shd_file.Nrr*sizeof(float));

    if (woss_ref.usingDebug()) {
      for (int i = 0; i < shd_file.Ntheta; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Ntheta(" << i << ") = " << shd_file.theta[i] << std::endl;
      }

      for (int i = 0; i < shd_file.Nsd; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Nsd(" << i << ") = " << shd_file.tx_depths[i] << std::endl;
      }

      for (int i = 0; i < shd_file.Nrd; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Nrd(" << i << ") = " << shd_file.rx_depths[i] << std::endl;
      }

      for (int i = 0; i < shd_file.Nrr; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Nrr(" << i << ") = " << shd_file.rx_ranges[i] << std::endl;
      }
    }
  }
  else if (bwoss_ref.getBellhopShdSyntax() == woss::BellhopShdSyntax::BELLHOP_CREATOR_SHD_FILE_SYNTAX_1) {
    file_reader.read(reinterpret_cast<char*>(&shd_file_v1.record_length),sizeof(int32_t));

    if (woss_ref.usingDebug()) 
      std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() record length = " << shd_file_v1.record_length << std::endl;

    file_reader.seekg(4*shd_file_v1.record_length, std::ios_base::beg); //reposition to end of first record
    shd_file_v1.plot_type = std::move( std::make_unique<char[]>(10*sizeof(char*)) );
    file_reader.read(shd_file_v1.plot_type.get(), 10*sizeof(char*));

    if (woss_ref.usingDebug()) 
      std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() plot type : " << shd_file_v1.plot_type.get() << std::endl;
    ...
    if ( strcmp( shd_file_v1.plot_type.get(), "rectilin  " ) == 0) {
        shd_file_v1.Nrx_per_range = shd_file_v1.Nrd;
    }
    else if ( strcmp( shd_file_v1.plot_type.get(), "irregular " ) == 0) {
    ...
    if (woss_ref.usingDebug()) std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() NFreq = " 
    ...
    shd_file_v1.frequencies = std::move( std::make_unique<double[]>(shd_file_v1.Nfreq) );
    shd_file_v1.theta = std::move( std::make_unique<double[]>(shd_file_v1.Ntheta) );
    shd_file_v1.tx_depths = std::move( std::make_unique<float[]>(shd_file_v1.Nsd) );
    shd_file_v1.rx_depths = std::move( std::make_unique<float[]>(shd_file_v1.Nrd) );
    shd_file_v1.rx_ranges = std::move( std::make_unique<double[]>(shd_file_v1.Nrr) );
    shd_file_v1.press_values = std::move( std::make_unique< std::complex<double>[] >(shd_file_v1.Nfreq * shd_file_v1.Ntheta * shd_file_v1.Nsd * shd_file_v1.Nrx_per_range * shd_file_v1.Nrr) );

    file_reader.seekg(3*4*shd_file_v1.record_length, std::ios_base::beg); //reposition to end of record 3
    file_reader.read(reinterpret_cast<char*>(shd_file_v1.frequencies.get()),shd_file_v1.Nfreq*sizeof(double));

    file_reader.seekg(4*4*shd_file_v1.record_length, std::ios_base::beg); //reposition to end of record 4
    file_reader.read(reinterpret_cast<char*>(shd_file_v1.theta.get()),shd_file_v1.Ntheta*sizeof(double));

    file_reader.seekg(7*4*shd_file_v1.record_length, std::ios_base::beg); //reposition to end of record 7
    file_reader.read(reinterpret_cast<char*>(shd_file_v1.tx_depths.get()),shd_file_v1.Nsd*sizeof(float));

    file_reader.seekg(8*4*shd_file_v1.record_length, std::ios_base::beg); //reposition to end of record 8
    file_reader.read(reinterpret_cast<char*>(shd_file_v1.rx_depths.get()),shd_file_v1.Nrd*sizeof(float));

    file_reader.seekg(9*4*shd_file_v1.record_length, std::ios_base::beg); //reposition to end of record 9
    file_reader.read(reinterpret_cast<char*>(shd_file_v1.rx_ranges.get()),shd_file_v1.Nrr*sizeof(double));

    if (woss_ref.usingDebug()) {
      for (int i = 0; i < shd_file_v1.Nfreq; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() NFreq(" << i << ") = " 
        << shd_file_v1.frequencies[i] << std::endl;
      }

      for (int i = 0; i < shd_file_v1.Ntheta; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Ntheta(" << i << ") = " << shd_file_v1.theta[i] << std::endl;
      }

      for (int i = 0; i < shd_file_v1.Nsd; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Nsd(" << i << ") = " << shd_file_v1.tx_depths[i] << std::endl;
      }

      for (int i = 0; i < shd_file_v1.Nrd; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Nrd(" << i << ") = " << shd_file_v1.rx_depths[i] << std::endl;
      }

      for (int i = 0; i < shd_file_v1.Nrr; i++) {
        std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdHeader() Nrr(" << i << ") = " << shd_file_v1.rx_ranges[i] << std::endl;
      }
    }
  }
...
bool ShdResReader::getShdFile()
{
 ...
 if (woss_ref.usingDebug())
    std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdFile() syntax " 
              << (int)bwoss_ref.getBellhopShdSyntax() << std::endl;

  if (bwoss_ref.getBellhopShdSyntax() == woss::BellhopShdSyntax::BELLHOP_CREATOR_SHD_FILE_SYNTAX_0) {
    auto press = std::make_unique<float[]>(2*shd_file.Nrr);

    for (int itheta = 0; itheta < shd_file.Ntheta; itheta++) {
      for (int isd = 0; isd < shd_file.Nsd; isd++) {
        if (woss_ref.usingDebug()) 
          std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdFile() Reading data for source " << isd << " of " 
        ...
        for (int ird = 0; ird < shd_file.Nrd; ird++) {
          ...
          if ( !file_reader.good()) {
            std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdFile() Seek to specified record failed in " 
                      << file_name << std::endl;
            exit(1);
          }
          file_reader.read(reinterpret_cast<char*>(press.get()), 2*shd_file.Nrr*sizeof(float));
          ...
          for (int i = 0; i < 2*shd_file.Nrr; i += 2 ) {
            ...
            if ( !std::isnan(real_part) && !std::isnan(imag_part) && !std::isinf(real_part) && !std::isinf(imag_part) ) {
              ...
              if (woss_ref.usingDebug()) 
                std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdFile() tx_depth = " 
    ...
    auto press = std::make_unique<float[]>(2*shd_file_v1.Nrr);
    ...
    for (int ifreq = 0; ifreq < shd_file_v1.Nfreq; ifreq++) {
      for (int itheta = 0; itheta < shd_file_v1.Ntheta; itheta++) {
        for (int isd = 0; isd < shd_file_v1.Nsd; isd++) {
          if (woss_ref.usingDebug()) 
            std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getShdFile() Reading data: f = "
          ...
          for (int ird = 0; ird < shd_file_v1.Nrd; ird++) {
            ...
            file_reader.read(reinterpret_cast<char*>(press.get()), 2*shd_file_v1.Nrr*sizeof(float));
            ...
            for (int i = 0; i < 2*shd_file_v1.Nrr; i += 2 ) {
              ...
              if ( !std::isnan(real_part) && !std::isnan(imag_part) && !std::isinf(real_part) && !std::isinf(imag_part) ) {
                ...
                if (woss_ref.usingDebug()) 
                  std::cout << "ShdResReader(" << woss_ref.getWossId() 
...
std::unique_ptr<Pressure> ShdResReader::readAvgPressure(double frequency, double tx_depth, double start_rx_depth, double start_rx_range, double end_rx_depth, double end_rx_range ) {
  if (!shd_file_collected) 
    return SDefHandler::instance().createPressure( Pressure::createNotValid() );
  return( SDefHandler::instance().createPressure( readMapAvgPressure(frequency, tx_depth, start_rx_depth, start_rx_range, end_rx_depth, end_rx_range ) ) ); 
}


std::unique_ptr<Pressure> ShdResReader::readPressure(double frequency, double tx_depth, double rx_depth, double rx_range ) const {
  if (!shd_file_collected) 
    return SDefHandler::instance().createPressure( Pressure::createNotValid() );
  return( SDefHandler::instance().createPressure( accessMap(frequency, tx_depth, rx_depth, rx_range) ) );
}


std::unique_ptr<TimeArr> ShdResReader::readTimeArr(double frequency, double tx_depth, double rx_depth, double rx_range ) const {
  if (!shd_file_collected ) 
    return SDefHandler::instance().createTimeArr( Pressure::createNotValid() );
  return( SDefHandler::instance().createTimeArr( accessMap(frequency, tx_depth, rx_depth, rx_range) ) ); 
} 
...
std::complex<double> ShdResReader::accessMap(double frequency, double tx_depth, double rx_depth, double rx_range, double theta ) const {
  ...
  if (bwoss_ref.getBellhopShdSyntax() == woss::BellhopShdSyntax::BELLHOP_CREATOR_SHD_FILE_SYNTAX_0) {
    ...
    std::cerr << "ShdResReader(" << woss_ref.getWossId() << ")::accessMap() unkown Shd syntax " << std::endl;
    exit(1);
  }
}
  ...
  if ( ( last_tx_depth == tx_depth ) && ( last_start_rx_depth == start_rx_depth ) && ( last_start_rx_range == start_rx_range ) 
   &&  ( last_end_rx_depth == end_rx_depth ) && ( last_end_rx_range == end_rx_range ) )
     ...
     if (bwoss_ref.getBellhopShdSyntax() == woss::BellhopShdSyntax::BELLHOP_CREATOR_SHD_FILE_SYNTAX_0) {
       ...
       std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::accessMap() unkown Shd syntax " << std::endl;
       exit(1);
     }
     ...
     if (woss_ref.usingDebug()) 
      std::cout << "ShdResReader(" << woss_ref.getWossId() << ")::getMapAvgPressure() sd = " << tx_depth 
