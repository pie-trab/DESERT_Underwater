...
static constexpr inline int GEBCO_1D_INVALID_INDEX = (-1);
static constexpr inline woss::Gebco2DIndexes GEBCO_2D_INVALID_INDEXES = woss::Gebco2DIndexes( GEBCO_1D_INVALID_INDEX, GEBCO_1D_INVALID_INDEX );
static constexpr inline double GEBCO_NOT_FOUND (-HUGE_VAL);


BathyGebcoDb::BathyGebcoDb( const std::string& name ) 
: WossNetcdfDb(name),
  gebco_type(GEBCO_BATHY_TYPE::GEBCO_2D_30_SECONDS_BATHY_TYPE),
...
bool BathyGebcoDb::finalizeConnection() {
    ...
    if ((gebco_type == GEBCO_BATHY_TYPE::GEBCO_1D_1_MINUTE_BATHY_TYPE) || (gebco_type == GEBCO_BATHY_TYPE::GEBCO_1D_30_SECONDS_BATHY_TYPE)) {
...
Bathymetry BathyGebcoDb::getValue( const Coord& coords ) const {
  ...
  if ((gebco_type == GEBCO_BATHY_TYPE::GEBCO_1D_1_MINUTE_BATHY_TYPE) || (gebco_type == GEBCO_BATHY_TYPE::GEBCO_1D_30_SECONDS_BATHY_TYPE)) {
  ...
  else if (    (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_1_MINUTE_BATHY_TYPE) 
            || (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_30_SECONDS_BATHY_TYPE) 
            || (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_15_SECONDS_BATHY_TYPE)) {
    ...
    if (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_1_MINUTE_BATHY_TYPE) {
    ...
    else if (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_30_SECONDS_BATHY_TYPE) {
    ...
    std::cout << "BathyGebcoDb::getValue() invalid gebco_type = " << static_cast<int>(gebco_type) << std::endl;
    return (HUGE_VAL);
  }
  ...
  if ( depth != GEBCO_NOT_FOUND && depth <= 0.0 ) 
...
long BathyGebcoDb::get1DBathyIndex( const Coord& coords ) const {
  if (gebco_type == GEBCO_BATHY_TYPE::GEBCO_1D_1_MINUTE_BATHY_TYPE) {
    double quantized_latitude = std::abs( coords.getLatitude() / GEBCO_1_MINUTE_BATHY_SPACING ); 
    if ( ( quantized_latitude - floor(quantized_latitude) ) >= 0.5) 
      quantized_latitude = ceil(quantized_latitude); 
    else 
      quantized_latitude = floor(quantized_latitude);
    ...
    if ( ( quantized_longitude - floor(quantized_longitude) ) >= 0.5) 
      quantized_longitude = ceil(quantized_longitude);
    else 
      quantized_longitude = floor(quantized_longitude);
  ...
  else if (gebco_type == GEBCO_BATHY_TYPE::GEBCO_1D_30_SECONDS_BATHY_TYPE) {
    ...
    std::cout << "BathyGebcoDb::get1DBathyIndex() ERROR wrong GEBCO type " << static_cast<int>(gebco_type) << std::endl;
...
Gebco2DIndexes BathyGebcoDb::get2DBathyIndexes( const Coord& coords ) const
{
  if (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_1_MINUTE_BATHY_TYPE) {
  ...
  else if (gebco_type == GEBCO_BATHY_TYPE::GEBCO_2D_15_SECONDS_BATHY_TYPE) {
    ...
    std::cout << "BathyGebcoDb::get2DBathyIndexes() ERROR wrong GEBCO type " << static_cast<int>(gebco_type) << std::endl;
