...
static constexpr inline int SSP_NOT_VALID = (-1000);


SspWoa2005Db::SspWoa2005Db( const std::string& name ) 
: WossNetcdfDb(name),
  woa_db_type(WOADbType::WOA_DB_TYPE_2005),
...
bool SspWoa2005Db::finalizeConnection() {
  if (woa_db_type == WOADbType::WOA_DB_TYPE_2005) {
#if defined (WOSS_NETCDF4_SUPPORT)
    ...
    return (ssp_var != nullptr);
#endif // defined (WOSS_NETCDF4_SUPPORT)
  }
#if defined (WOSS_NETCDF4_SUPPORT)
  else if (woa_db_type == WOADbType::WOA_DB_TYPE_2013) {
    ssp_var = netcdf_db->getVar("ssp");
    if (ssp_var.isNull()) {
...
SSPIndexes SspWoa2005Db::getSSPIndexes( const Coord& coordinates ) const {
  ...
  if (debug) 
    std::cout << "SspWoa2005Db::getSSPIndexes() 2005 latitude = " << coordinates.getLatitude() << "; longitude = " 
...
#endif // defined (WOSS_NETCDF4_SUPPORT)
{
  if (woa_db_type == WOADbType::WOA_DB_TYPE_2005) {
    ...
    if (lat == SSP_NOT_VALID) {
      std::cerr << "SspWoa2005Db::getSSPValue() 2013 Couldn't extract current lat" << std::endl;
      exit(1);
    }
    ...
    if (lon == SSP_NOT_VALID) {
      std::cerr << "SspWoa2005Db::getSSPValue() 2013 Couldn't extract current lon" << std::endl;
      exit(1);
    }
...
std::unique_ptr<SSP> SspWoa2005Db::getValue( const Coord& coordinates, const Time& time, long double ssp_depth_precision ) const {
  double curr_ssp[SSP_STD_NDEPTH];
  DepthMap ssp_map;
  SSPIndexes curr_ind = getSSPIndexes(coordinates);
  
  if(debug) 
    std::cout << "SspWoa2005Db::getValue() coordinates = " << coordinates << "; indexes = " << curr_ind.first << " , " 
  ...
  for (int i = 0; i < SSP_STD_NDEPTH; i++) {
    ...
    if(debug)
      std::cout << "SspWoa2005Db::getValue() depth = " << ssp_std_depths[i] << "; ssp value = " << curr_ssp[i] << std::endl;
    ...
    ssp_map.emplace( std::make_pair( PDouble( ssp_std_depths[i], ssp_depth_precision ), curr_ssp[i] ) );
  }

  return( SDefHandler::instance().createSSP( ssp_map, ssp_depth_precision ) );
}
