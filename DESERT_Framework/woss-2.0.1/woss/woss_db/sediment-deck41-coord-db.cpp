...
static constexpr inline int SEDIMENT_NOT_FOUND = (-100000000);
...
: WossNetcdfDb( name ),
  ...
  lon_var(),
#else
  main_sedim_var_coord(nullptr),
  sec_sedim_var_coord(nullptr),
  lat_var(nullptr),
  lon_var(nullptr),
#endif // defined (WOSS_NETCDF4_SUPPORT)
  deck41_db_type(DECK41DbType::DECK41_DB_V1_TYPE)
{
}
...
bool SedimDeck41CoordDb::finalizeConnection() {
  assert(deck41_db_type != DECK41DbType::DECK41_DB_INVALID_TYPE);
  ...
  if (deck41_db_type == DECK41DbType::DECK41_DB_V2_TYPE) {
  ...
  return ( (main_sedim_var_coord != nullptr) && (sec_sedim_var_coord != nullptr) );
#endif // defined (WOSS_NETCDF4_SUPPORT)
}
...
Deck41Types SedimDeck41CoordDb::getSeaFloorType( const Coord& coordinates ) const {
  ...
  if (deck41_db_type == DECK41DbType::DECK41_DB_V1_TYPE)
  {
#if defined (WOSS_NETCDF4_SUPPORT)
  ...
  if (main_type == SEDIMENT_NOT_FOUND) {
    std::cerr << "SedimDeck41CoordDb::getSeaFloorType() Couldn't extract current main_type" << std::endl;
    exit(1);
  }

  sec_sedim_var_coord.getVar(index_vector, &secondary_type);
  if (secondary_type == SEDIMENT_NOT_FOUND) {
    std::cerr << "SedimDeck41CoordDb::getSeaFloorType() Couldn't extract current secondary_type" << std::endl;
    exit(1);
  }
#else
  NcBool ret_val = main_sedim_var_coord->set_cur(index);
  if (!ret_val) {
    std::cerr << "SedimDeck41CoordDb::getSeaFloorType() Couldn't set_cur() of main_sedim_var_coord;" 
  ...
  if (!ret_val) {
    std::cerr << "SedimDeck41CoordDb::getSeaFloorType() Couldn't set_cur() of sec_sedim_var_coord;"
	      << " current index = " << index << std::endl;
    exit(1);
  }
...
#if defined (WOSS_NETCDF4_SUPPORT)
  else if (deck41_db_type == DECK41DbType::DECK41_DB_V2_TYPE)
  {
    ...
    if (lat == SEDIMENT_NOT_FOUND) {
      std::cerr << "SedimDeck41CoordDb::getSeaFloorType() Couldn't extract current lat" << std::endl;
      exit(1);
    }
    ...
    if (lon == SEDIMENT_NOT_FOUND) {
      std::cerr << "SedimDeck41CoordDb::getSeaFloorType() Couldn't extract current lon" << std::endl;
      exit(1);
    }
    ...
    std::cerr << "SedimDeck41CoordDb::getSeaFloorType() invalid deck41_db_type = " << (int)deck41_db_type << std::endl;
    exit(1);
  }

  if (debug) 
    std::cout << "SedimDeck41CoordDb::getSeaFloorType() lat = " << coordinates.getLatitude() << "; lon = " 
