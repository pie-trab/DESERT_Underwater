...
static constexpr inline int SEDIMENT_NOT_FOUND = (-1000000);
...
#else
  main_sedim_var_marsden_one(nullptr),
  sec_sedim_var_marsden_one(nullptr),
  marsden_square_var(nullptr),
  marsden_one_square_var(nullptr),
...
bool SedimDeck41MarsdenOneDb::finalizeConnection() {
  ...
  return ( (main_sedim_var_marsden_one != nullptr) && (sec_sedim_var_marsden_one != nullptr) );
#endif // defined (WOSS_NETCDF4_SUPPORT)
}
...
Deck41Types SedimDeck41MarsdenOneDb::getSeaFloorType(const MarsdenCoord& marsden_coord) const {
  ...
  if (debug) 
    std::cout << "SedimDeck41MarsdenOneDb::getSeaFloorType() marsden square = " << marsden_coord.first 
  ...
  else if (deck41_db_type == DECK41DbType::DECK41_DB_V1_TYPE) {
