...
static constexpr inline int SEDIMENT_NOT_FOUND = (-1000);
...
: WossNetcdfDb( name ),
  ...
  marsden_square_var(),
#else
  main_sedim_var_marsden(nullptr),
  sec_sedim_var_marsden(nullptr),
...
bool SedimDeck41MarsdenDb::finalizeConnection() {
  ...
  return ( (main_sedim_var_marsden != nullptr) && (sec_sedim_var_marsden != nullptr) );
#endif // defined (WOSS_NETCDF4_SUPPORT)
}
...
Deck41Types SedimDeck41MarsdenDb::getSeaFloorType( const Marsden& marsden_square ) const {
  ...
  if (debug) 
    std::cout << "SedimDeck41MarsdenDb::getSeaFloorType() marsden square = " << marsden_square << std::endl;
