...
namespace woss {
  ...
  class WossException : public std::exception {
   ...
   /**
     ...
     **/
    static constexpr inline int CHAR_ARR_LENGTH = 250;
   ...
   /**
    ...
    /**
      * destructor
      **/
    virtual ~WossException() throw () override = default;

    /**
      * assignement operator
      * @param copy the source copy
      **/
    WossException& operator=(const WossException& copy) = default;
  ...
  #define WOSS_EXCEPTION_INFO(err_type, info) throw WossException(err_type, __FILE__, __LINE__, __FUNCTION__, info)

  #define WOSS_EXCEPTION(err_type) throw WossException(err_type, __FILE__, __LINE__, __FUNCTION__, "")
  ...
  class TestTimeReference: public TimeReference {
    ...
    **/
     ...
     * @return a std::unique_ptr to a heap-allocated copy of <b>this</b> instance
     **/
    virtual std::unique_ptr<TimeReference> clone() const override { return std::make_unique<TestTimeReference>(*this); }
  ...
  class WossTest {
    ...
    **/
     ...
     * @param tmp rvalue reference to a WossTest object
     **/
     WossTest(WossTest&& tmp) = delete;

     /**
     * Deleted move assignement operator
     * @param copy rvalue reference to a WossTest object
     **/
     WossTest& operator=(const WossTest& copy) = delete;

     /**
     * Deleted assignement operator
     * @param tmp const reference to a WossTest object
     **/
  ...
  /**
    ...
    **/
    ...
    /**
    * Sets the Woss channel simulator bellhop binary name
    * @param name bellhop binary name
    * @return reference to this instance
    **/
    WossTest& setWossBellhopBinName(const std::string& name) { bellhop_bin_name = name; return *this; }

    /**
    * Gets the Woss channel simulator bellhop binary name
    * @return bellhop binary name
    **/
    std::string getWossBellhopBinName() const { return bellhop_bin_name; }
    ...
    * @return reference to this instance
    **/
    WossTest& setWossManagerThreads(unsigned int threads) { concurrent_threads = threads; return *this; }
    ...
    * @param flag use threadpool flag
    * @return reference to this instance
    **/
    WossTest& setWossManagerUseThreadPool(bool flag) { woss_manager_use_thread_pool = flag; return *this; }

    /**
    * Gets the WossManager thread pool flag
    * @return use thread pool flag
    **/
    bool getWossManagerUseThreadPool() const { return woss_manager_use_thread_pool; }
    ...
    **/
    ...
    std::unique_ptr<SSP> ssp_proto; //!< SSP prototype which will be plugged into the WOSS framework.
    std::unique_ptr<Sediment> sediment_proto; //!< Sediment prototype which will be plugged into the WOSS framework.
    std::unique_ptr<Pressure> pressure_proto; //!< Pressure prototype which will be plugged into the WOSS framework.
    std::unique_ptr<TimeArr> time_arr_proto; //!< TimeArr prototype which will be plugged into the WOSS framework.
    std::unique_ptr<Transducer> transducer_proto; //!< Transducer prototype which will be plugged into the WOSS framework.
    std::unique_ptr<AltimBretschneider> altim_bret_proto; //!< AltimBretschneider prototype which will be plugged into the WOSS framework.
    std::unique_ptr<Location> location_proto; //!< Location prototype which will be plugged into the WOSS framework.
    int woss_random_gen_stream; //!< random generator used by the WOSS framework.
    std::unique_ptr<RandomGenerator> random_gen_proto; //!< RandomGenerator prototype which will be plugged into the WOSS framework.
    std::unique_ptr<TimeReference> time_ref_proto; //!< TimeReference prototype which will be plugged into the WOSS framework.
    ...
    std::shared_ptr<ResPressureBinDbCreator> res_db_creator_press_bin; //!< the helper will automatically allocate the desired result database creator based on current configuration.
    std::shared_ptr<ResPressureTxtDbCreator> res_db_creator_press_txt; //!< the helper will automatically allocate the desired result database creator based on current configuration.
    std::shared_ptr<ResTimeArrBinDbCreator> res_db_creator_time_arr_bin; //!< the helper will automatically allocate the desired result database creator based on current configuration.
    std::shared_ptr<ResTimeArrTxtDbCreator> res_db_creator_time_arr_txt; //!< the helper will automatically allocate the desired result database creator based on current configuration.
    ...
    std::shared_ptr<SedimDeck41DbCreator> sedim_db_creator; //!< the helper will automatically allocate the woss sediment database creator
...
#endif // defined (WOSS_NETCDF_SUPPORT)
    std::shared_ptr<SspWoa2005DbCreator> ssp_db_creator; //!< the helper will automatically allocate the woss sediment database creator
    ...
    std::shared_ptr<BathyGebcoDbCreator> bathy_db_creator; //!< the helper will automatically allocate the woss GEBCO bathymetry db creator
#endif // defined (WOSS_NETCDF_SUPPORT)

    bool woss_db_manager_debug; //!< enable/disable the debug prints of the woss DB manager object.

    std::shared_ptr<WossDbManager> woss_db_manager; //!< the helper will automatically allocate the woss DB manager
    ...
    std::string bellhop_bin_name;  //!< woss object configuration: Bellhop binary name
    ...
    std::shared_ptr<BellhopCreator> bellhop_creator; //!< the helper will automatically allocate the woss creator
    ...
    unsigned int concurrent_threads; //!< number of concurrent threads (0 = auto).
    ...
    bool woss_manager_use_thread_pool; //!< enable/disable woss::ThreadPool usage
    std::shared_ptr< WossManagerSimple<WossManagerResDb> > woss_manager_simple; //!<  the helper will automatically allocate the desired woss manager based on current configuration.
    std::shared_ptr< WossManagerSimple<WossManagerResDbMT> > woss_manager_multi; //!<  the helper will automatically allocate the desired woss manaeger based on current configuration.

    bool woss_transducer_handler_debug; //!< enable/disable the debug prints of the woss transducer handler.
    std::shared_ptr<TransducerHandler> woss_transducer_handler; //!< the helper will automatically allocate the woss Transducer handler.

    bool woss_controller_debug; //!< enable/disable the debug prints of the woss conteroller
    std::shared_ptr<WossController> woss_controller; //!< the helper will automatically allocate the woss controller.
