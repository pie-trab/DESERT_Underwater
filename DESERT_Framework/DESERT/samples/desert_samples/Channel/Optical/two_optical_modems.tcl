...
set opt(inclination_0)      3       ;# rad: node 0 points toward +X
...
set opt(beam_lut_path)      [file join $opt(db_dir) bluecomm beam_pattern beam5mbps.csv]
set opt(max_range_lut_path) [file join $opt(db_dir) bluecomm max_range max_range5mbps.csv]
set opt(noise_lut_path)     [file join $opt(db_dir) optical_noise LUT.txt]
...
proc createNode {id} {
    ...
    if {$id == 0} {
        $phy($id) setInclinationAngle $opt(inclination_0)
    } else {
        $phy($id) setInclinationAngle $opt(inclination_1)
    }
...
# sources are left unstarted, so this remains a two-modem point-to-point test.
